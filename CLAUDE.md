...
bun run preflight:full   # + mock-e2e, dependency-scan, container, image-scan, compose-e2e, systemd-native
bun run ci:status        # poll the PR's checks, then dump failing job logs
```

`test.yml` has eight jobs. `--full` covers `quality`, `mock-e2e`,
`dependency-scan`, `container`, `image-scan`, `compose-e2e`, and
`systemd-native`. The three Docker jobs skip with a notice when no daemon is
running, and `systemd-native` skips unless it has root and systemd as PID 1.
It does **not** cover `secret-scan`, which is a gitleaks GitHub Action with no
local invocation.
