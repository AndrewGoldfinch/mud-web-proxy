...
// Mock SocketExtended interface
interface MockSocketExtended {
  req: IncomingMessage & { connection: { remoteAddress: string } };
  ts: MockTelnetSocket | undefined;
  host?: string;
  port?: number;
  ttype: string[];
  name?: string;
...
// Mock TelnetSocket interface
interface MockTelnetSocket {
...
function createMockTelnetSocket(
  overrides: Partial<MockTelnetSocket> = {},
): MockTelnetSocket {
  return {
    write: () => true,
    send: () => {},
...
function createIACSequence(command: number, option: number): Buffer {
...
class MockServer {
  ...
  writes: { socket: MockSocketExtended; data: Buffer }[] = [];

  sendTTYPE(s: MockSocketExtended, msg: string): void {
    if (msg && s.ts) {
      const p = this.prt;
  ...
  sendGMCP(s: MockSocketExtended, msg: string): void {
    if (s.ts) {
  ...
  sendMSDP(
    s: MockSocketExtended,
    msdp: { key?: string; val?: string | string[] },
  ): void {
    const p = this.prt;
    if (!msdp.key || !msdp.val || !s.ts) return;
  ...
  sendMSDPPair(s: MockSocketExtended, key: string, val: string): void {
    const p = this.prt;
    if (!s.ts) return;
  ...
  recordWrite(s: MockSocketExtended, data: Buffer): void {
  ...
  sendClient(s: MockSocketExtended, data: Buffer): void {
    const p = this.prt;
...
describe('Telnet Negotiation Handlers - Part 1', () => {
  let srv: MockServer;
  let mockSocket: MockSocketExtended;
  let mockTelnetSocket: MockTelnetSocket;
  ...
  describe('TTYPE (Terminal Type) Negotiation', () => {
    ...
    test('No terminal type sent when ttype array is empty', () => {
      ...
      const writes = srv.writes.filter((w) => w.socket === mockSocket);
      const ttypeWrites = writes.filter(
        (w) =>
          w.data[0] === PROTOCOL.IAC &&
          (w.data[2] === PROTOCOL.TTYPE ||
  ...
  describe('GMCP (Generic MUD Communication Protocol) Negotiation', () => {
    ...
    test('Client IP sent via GMCP', () => {
      ...
      const clientIPMessage = gmcpMessages.find((m) =>
        m.startsWith('client_ip '),
      );
  ...
  describe('sendMSDP function', () => {
    ...
    test('sendMSDP with single value', () => {
      ...
      expect(writes[4]!.data).toEqual(
        Buffer.from([PROTOCOL.IAC, PROTOCOL.SE]),
      );
    });
  ...
  describe('Protocol sequence verification', () => {
    test('Complete TTYPE negotiation sequence', () => {
      ...
      const willTTYPEIndex = writes.findIndex(
        (w) =>
          w.data.length === 3 &&
          w.data[0] === PROTOCOL.IAC &&
          w.data[1] === PROTOCOL.WILL &&
      ...
      const sbTTYPEIndex = writes.findIndex(
        (w) =>
          w.data.length === 4 &&
          w.data[0] === PROTOCOL.IAC &&
      ...
      const seIndex = writes.findIndex(
        (w) =>
          w.data.length === 2 &&
          w.data[0] === PROTOCOL.IAC &&
  ...
  describe('Multiple negotiation scenarios', () => {
    test('TTYPE, GMCP, and MSDP all negotiate correctly', () => {
      ...
      const hasTTYPE = writes.some(
        (w) =>
          w.data[0] === PROTOCOL.IAC &&
          w.data[1] === PROTOCOL.WILL &&
          w.data[2] === PROTOCOL.TTYPE,
      );
      const hasGMCP = writes.some(
        (w) =>
          w.data[0] === PROTOCOL.IAC &&
          w.data[1] === PROTOCOL.WILL &&
          w.data[2] === PROTOCOL.GMCP,
      );
      const hasMSDP = writes.some(
        (w) =>
          w.data[0] === PROTOCOL.IAC &&
          w.data[1] === PROTOCOL.DO &&
          w.data[2] === PROTOCOL.MSDP,
