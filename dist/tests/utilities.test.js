...
describe('stringify() function', () => {
    const stringify = function (A) {
        const cache = [];
        const val = JSON.stringify(A, function (_k, v) {
...
describe('loadF() dynamic reload', () => {
    ...
    test('should handle minification errors', () => {
        ...
        expect(capturedLogMessages.some((msg) => msg.includes('Minify/load error'))).toBe(true);
    });
    ...
    test('should log success and errors appropriately', () => {
        ...
        expect(capturedLogMessages.some((msg) => msg.includes('error'))).toBe(false);
    });
});
...
describe('die() function', () => {
    ...
    const die = function (core, sockets, srvOpen) {
...
describe('Server initialization', () => {
    ...
    test('should load chat log on startup', async () => {
        ...
        fs.writeFileSync(path.join(testDir, 'chat.json'), JSON.stringify(chatEntries));
        const loadChatLog = async () => {
            try {
                const data = await fs.promises.readFile(path.join(testDir, 'chat.json'), 'utf8');
                const parsed = JSON.parse(data);
                return Array.isArray(parsed) ? parsed : [];
            }
            catch {
    ...
    test('should set up file watching on wsproxy.js', () => {
        ...
        expect(fs.readFileSync(path.join(testDir, 'wsproxy.js'), 'utf8')).toBe('// updated content');
        fs.rmSync(testDir, { recursive: true, force: true });
    });
...
describe('Cache management for circular detection', () => {
    const stringifyWithCache = function (A) {
        const cache = [];
        const val = JSON.stringify(A, function (_k, v) {
            if (typeof v === 'object' && v !== null) {
                if (cache.indexOf(v) !== -1)
...
describe('loadChatLog edge cases', () => {
    ...
    test('should handle large chat files', async () => {
        ...
        fs.writeFileSync(path.join(testDir, 'chat.json'), JSON.stringify(largeEntries));
        const result = await loadChatLog();
