...
function createTelnetCommand(...bytes) {
    return Buffer.from(bytes);
}
// Helper to find command sequence in buffer
function findSequenceInBuffer(buffer, sequence) {
...
function handleMXPNegotiation(s, data) {
    const p = TELNET;
    if (!s.mxp_negotiated) {
...
function handleMCCPNegotiation(s, data) {
    const p = TELNET;
...
function handleCharsetNegotiation(s, data) {
    const p = TELNET;
...
function handleNewEnvNegotiation(s, data) {
    const p = TELNET;
    if (!s.new_negotiated) {
        for (let i = 0; i < data.length; i++) {
...
function handleBasicNegotiations(s, data) {
    const p = TELNET;
    if (!s.sga_negotiated) {
        for (let i = 0; i < data.length; i++) {
            if (data[i] === p.IAC &&
...
describe('MXP (MUD eXtension Protocol) Negotiation', () => {
    ...
    describe('IAC DO MXP', () => {
        it('should respond with IAC WILL MXP when receiving IAC DO MXP', () => {
            ...
            expect(result.response).toEqual(Buffer.from([TELNET.IAC, TELNET.WILL, TELNET.MXP]));
            const writtenData = mockSocket.ts.getWrittenData();
            expect(writtenData).toEqual(Buffer.from([TELNET.IAC, TELNET.WILL, TELNET.MXP]));
        });
    ...
    describe('IAC WILL MXP', () => {
        it('should respond with IAC DO MXP when receiving IAC WILL MXP', () => {
            ...
            const result = handleMXPNegotiation(mockSocket, data);
            expect(result.handled).toBe(true);
            expect(result.response).toEqual(Buffer.from([TELNET.IAC, TELNET.DO, TELNET.MXP]));
            const writtenData = mockSocket.ts.getWrittenData();
            expect(writtenData).toEqual(Buffer.from([TELNET.IAC, TELNET.DO, TELNET.MXP]));
        });
    ...
    describe('MXP Tag Escaping', () => {
        it('should handle ESC[1z for opening MXP tags', () => {
            ...
            expect(fullMessage.indexOf(tagClose)).toBe(tagOpen.length + tagContent.length);
        });
...
describe('CHARSET/UTF-8 Negotiation', () => {
    ...
    describe('IAC DO CHARSET', () => {
        it('should respond with IAC WILL CHARSET when receiving IAC DO CHARSET', () => {
            ...
            expect(result.response).toEqual(Buffer.from([TELNET.IAC, TELNET.WILL, TELNET.CHARSET]));
        });
    ...
    describe('UTF-8 Negotiation Flag Management', () => {
        it('should track utf8_negotiated state correctly', () => {
            ...
            const doData = createTelnetCommand(TELNET.IAC, TELNET.DO, TELNET.CHARSET);
            handleCharsetNegotiation(mockSocket, doData);
            expect(mockSocket.utf8_negotiated).toBe(0);
            // SB CHARSET - now negotiated
            const sbData = createTelnetCommand(TELNET.IAC, TELNET.SB, TELNET.CHARSET);
...
describe('NEW-ENV (New Environment) Negotiation', () => {
    ...
    describe('IAC DO NEW', () => {
        it('should respond with IAC WILL NEW when receiving IAC DO NEW', () => {
            ...
            expect(result.handled).toBe(true);
            expect(result.response).toEqual(Buffer.from([TELNET.IAC, TELNET.WILL, TELNET.NEW]));
            const writtenData = mockSocket.ts.getWrittenData();
            expect(writtenData).toEqual(Buffer.from([TELNET.IAC, TELNET.WILL, TELNET.NEW]));
        });
...
describe('SGA, ECHO, NAWS Handling', () => {
    ...
    describe('IAC WILL SGA', () => {
        it('should respond with IAC WONT SGA when receiving IAC WILL SGA', () => {
            ...
            expect(result.response).toEqual(Buffer.from([TELNET.IAC, TELNET.WONT, TELNET.SGA]));
        });
    ...
    describe('IAC WILL NAWS', () => {
        it('should respond with IAC WONT NAWS when receiving IAC WILL NAWS', () => {
            ...
            expect(result.response).toEqual(Buffer.from([TELNET.IAC, TELNET.WONT, TELNET.NAWS]));
        });
    ...
    describe('Protocol Compliance', () => {
        it('should handle multiple negotiations in sequence', () => {
            ...
            mockSocket.ts.clearWrittenData();
            // ECHO
            const echoData = createTelnetCommand(TELNET.IAC, TELNET.WILL, TELNET.ECHO);
            handleBasicNegotiations(mockSocket, echoData);
            mockSocket.ts.clearWrittenData();
            // NAWS
            const nawsData = createTelnetCommand(TELNET.IAC, TELNET.WILL, TELNET.NAWS);
        ...
        it('should respect the negotiated flags to prevent duplicate responses', () => {
            ...
            const negotiations = [
                {
                    cmd: [TELNET.IAC, TELNET.WILL, TELNET.SGA],
                    flag: 'sga_negotiated',
                },
                {
                    cmd: [TELNET.IAC, TELNET.WILL, TELNET.ECHO],
                    flag: 'echo_negotiated',
                },
                {
                    cmd: [TELNET.IAC, TELNET.WILL, TELNET.NAWS],
                    flag: 'naws_negotiated',
                },
            ];
...
describe('Protocol Compliance Tests', () => {
    ...
    describe('Command Sequence Validation', () => {
        it('should create valid IAC WILL MXP sequence', () => {
            const sequence = createTelnetCommand(TELNET.IAC, TELNET.WILL, TELNET.MXP);
...
//# sourceMappingURL=telnet-negotiation-part2.test.js.map