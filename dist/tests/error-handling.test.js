...
const createMockTelnetSocket = (overrides = {}) => {
    const listeners = {};
    const mockSocket = {
        write: jest.fn(() => true),
        send: jest.fn(),
...
const sendClient = (s, data) => {
    if (s.mccp && !s.mccp_negotiated && !s.compressed) {
        for (let i = 0; i < data.length; i++) {
            if (data[i] === PROTOCOL.IAC &&
...
describe('Error Handling', () => {
    ...
    describe('Network Errors', () => {
        ...
        it('should handle telnet connection timeout', () => {
            ...
            expect(mockSendClient).toHaveBeenCalledWith(s, Buffer.from('Timeout: server port is down.\r\n'));
        });
        ...
        it('should handle WebSocket error', () => {
            ...
            const msg = new Date().toISOString() +
                ...
                error;
            srv.log(msg);
            expect(mockLog).toHaveBeenCalledWith(expect.stringContaining('(ws) peer'));
    ...
    describe('Timeout Handling', () => {
        it('should trigger cleanup on telnet socket timeout', () => {
            ...
            expect(mockLog).toHaveBeenCalledWith(expect.stringContaining('telnet socket timeout:'));
        });
        ...
        it('should close socket after timeout', (done) => {
            const s = createMockSocket();
            const ts = createMockTelnetSocket();
            s.ts = ts;
    ...
    describe('Compression Errors', () => {
        it('should handle zlib deflateRaw error', () => {
            ...
            zlib.deflateRaw =
                jest.fn((_data, callback) => {
                    callback(new Error('Compression failed'), Buffer.from(''));
                });
            sendClient(s, data);
            // Wait for async callback
            setTimeout(() => {
                expect(mockLog).toHaveBeenCalledWith(expect.stringContaining('zlib error:'));
                // Restore original
    ...
    describe('Encoding Errors', () => {
        it('should handle iconv encoding failure', () => {
            ...
            expect(mockLog).not.toHaveBeenCalledWith(expect.stringContaining('error:'));
        });
        it('should log encoding errors', () => {
            ...
            expect(mockLog).toHaveBeenCalledWith(expect.stringContaining('error:'), s);
        });
        it('should handle UTF-8 to latin1 conversion edge cases', () => {
            ...
            try {
                ...
                srv.log('Encoding error: ' + ex.toString(), s);
            }
        });
    });
    describe('File Operation Errors', () => {
        it('should handle chat log file read error', async () => {
            ...
            const nonExistentPath = '/tmp/nonexistent-chat-log-' + Date.now() + '.json';
            ...
            expect(mockLog).toHaveBeenCalledWith(expect.stringContaining('Chat log error:'));
        });
        ...
        it('should handle eval() errors in dynamic reload', () => {
            ...
            expect(mockLog).toHaveBeenCalledWith(expect.stringContaining('Minify/load error:'));
        });
    ...
    describe('Edge Cases', () => {
        ...
        it('should handle socket close event when already closing', () => {
            ...
            expect(mockLog).toHaveBeenCalledWith(expect.stringContaining('telnet socket closed'));
        });
