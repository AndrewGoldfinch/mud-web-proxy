...
class MockTelnetServer {
    ...
    port;
    host;
    constructor(port = TEST_CONFIG.tnPort, host = TEST_CONFIG.tnHost) {
        this.port = port;
        this.host = host;
    }
    async start() {
        ...
        for (let i = 0; i < data.length; i++) {
            if (data[i] === TELNET.IAC && i + 2 < data.length) {
                ...
                if (cmd === TELNET.DO && opt === TELNET.MSDP) {
                    ...
                    // Send some MSDP data
                    setTimeout(() => {
                        const msdpData = createSubnegotiation(TELNET.MSDP, TELNET.MSDP_VAR, ...Buffer.from('SERVER_ID'), TELNET.MSDP_VAL, ...Buffer.from('TestServer'));
...
describe('Integration Tests - Full WebSocket-to-Telnet Flow', () => {
    ...
    describe('1. Full WebSocket-to-Telnet Flow', () => {
        ...
        test('8. Proxy forwards to WebSocket client as base64', async () => {
            ...
            expect(Buffer.from(base64Data, 'base64').toString()).toBe('Hello from MUD!');
    ...
    describe('2. Protocol Negotiation Flow', () => {
        describe('TTYPE Negotiation', () => {
            test('13. TTYPE negotiation complete flow', async () => {
                ...
                const requestTtype = createSubnegotiation(TELNET.TTYPE, TELNET.REQUEST);
                ttypeSequence.push(requestTtype);
                // Step 4: Client sends terminal type
                const terminalType = createSubnegotiation(TELNET.TTYPE, TELNET.IS, ...Buffer.from('xterm-256color'));
        ...
        describe('GMCP Negotiation', () => {
            test('14. GMCP negotiation complete flow', async () => {
                ...
                const gmcpData = createSubnegotiation(TELNET.GMCP, ...Buffer.from('client test-client'));
        ...
        describe('MSDP Variable Exchange', () => {
            test('16. MSDP variable exchange', async () => {
                ...
                clientVars.forEach(({ key, val }) => {
                    const msdpVar = createSubnegotiation(TELNET.MSDP, TELNET.MSDP_VAR, ...Buffer.from(key), TELNET.MSDP_VAL, ...Buffer.from(val));
    ...
    describe('5. Data Transformation End-to-End', () => {
        ...
        test('26. Compressed data flow', async () => {
            ...
            const testData = Buffer.from('This is a test message for compression. '.repeat(10));
            // Compress
