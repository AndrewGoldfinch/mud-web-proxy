...
describe('Client Request Parsing', () => {
    ...
    describe('Field Configuration Tests', () => {
        it('should set s.host from req.host', () => {
            ...
            expect(mockLog).toHaveBeenCalledWith('Target host set to example.com', s);
        });
        ...
        it('should set s.ttype array from req.ttype', () => {
            ...
            expect(mockLog).toHaveBeenCalledWith('Client ttype set to XTERM-256color', s);
        });
    ...
    describe('Command Handling Tests', () => {
        it('should call srv.chat() when req.chat is set', () => {
            const s = createMockSocket();
            const data = Buffer.from('{ "chat": 1, "channel": "general", "msg": "Hello" }');
            const result = parse(s, data);
            expect(result).toBe(1);
        ...
        it('should call srv.sendMSDP() when req.msdp is set', () => {
            ...
            const msdpData = { key: 'TestKey', val: 'TestValue' };
            ...
            expect(mockLog).toHaveBeenCalledWith('Attempt msdp send: ' + JSON.stringify(msdpData));
        });
    ...
    describe('Missing Optional Fields Tests', () => {
        ...
        it('should handle request with multiple fields set', () => {
            const s = createMockSocket();
            const data = Buffer.from('{ "host": "test.com", "port": 7000, "name": "User", "client": "WebClient" }');
            const result = parse(s, data);
            expect(result).toBe(1);
            expect(s.host).toBe('test.com');
    ...
    describe('Edge Cases', () => {
        it('should handle multiple commands in single request', () => {
            ...
            const data = Buffer.from('{ "host": "test.com", "port": 7000, "connect": 1, "chat": 1 }');
            const result = parse(s, data);
            expect(result).toBe(1);
            expect(s.host).toBe('test.com');
            expect(s.port).toBe(7000);
        ...
        it('should handle boolean false values (truthy check skips falsy)', () => {
            const s = createMockSocket();
            const data = Buffer.from('{ "mccp": false, "utf8": false, "debug": false }');
            const result = parse(s, data);
            expect(result).toBe(1);
        ...
        it('should handle special characters in strings', () => {
            const s = createMockSocket();
            const data = Buffer.from('{ "name": "User\\"Test", "host": "test\\nhost" }');
            const result = parse(s, data);
            expect(result).toBe(1);
...
//# sourceMappingURL=client-request.test.js.map