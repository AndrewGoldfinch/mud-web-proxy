...
const srv = {
    ...
    init: async function () {
        ...
        fs.watch(srv.path + '/wsproxy.js', function (_event, filename) {
            if (filename === null || typeof filename !== 'string')
                return;
            const key = 'update-' + filename;
            if (srv[key])
                clearTimeout(srv[key]);
            srv[key] =
                setTimeout(function () {
                    srv.loadF(filename);
                }, 1000);
        ...
        s.ts.send = function (data) {
            if (srv.debug) {
                ...
                for (let i = 0; i < data.length; i++)
                    raw.push(util.format('%d', typeof data === 'string' ? data.charCodeAt(i) : data[i]));
    ...
    log: function (msg, s) {
        ...
        console.log(util.format(new Date().toISOString() + ' %s: %s', s.req.connection.remoteAddress, msg));
    },
    die: function (core) {
        ...
        for (let i = 0; i < ss.length; i++) {
            ...
            if (ss[i] &&
                ss[i].write)
                ss[i].write('Proxy server is going down...');
