...
- Client-to-proxy TLS and proxy-to-MUD TLS are independent. Outbound plain never
  attempts TLS; prefer attempts TLS and may downgrade exactly once, on a
  classified TLS negotiation error, a peer close during the handshake, the
  four-second handshake deadline, or certificate validation failure; required
  never falls back to plaintext under any of those.
- One shared connector serves typed and legacy connections, so `MUD_TLS_MODE`
  governs both identically. The four-second handshake deadline is distinct from
  the twelve-second Telnet negotiation timer, which starts only after connect.
```
...
for path in \
  ...
  tests/mud-transport.test.ts tests/tls-servername.test.ts \
