...
every consideration in this document. Configuration alone decides whether an
installation is a MUD gateway or a general-purpose open relay.

This document explains what the built-in controls actually do, what they don't
do, and what an operator accepts by loosening each default. It is the
...
the [Configuration reference](configuration.md), and this document doesn't
restate that table. Deployment specifics live in
[Docker Compose deployment](deployment/compose.md) and
[Native systemd deployment](deployment/systemd.md).
...
resolved address, and forwards bytes in both directions until one side closes the
connection. It speaks the Telnet option protocol upstream and frames MUD output for
...
- **The proxy is an outbound connection source.** Every request a client makes
  ...
  names, including hosts inside the operator's own network.
- **MUD output is untrusted input.** The proxy relays what the MUD sends. It
  doesn't sanitize game output, and it can't: escape sequences, markup, and
...
HTTPS and WSS. Certificates, protocol versions, and public exposure are Caddy's,
...
`ALLOW_INSECURE_INBOUND_NO_TLS` acknowledgment, which exists so that plaintext
...
**Application to MUD.** `MUD_TLS_MODE` governs this hop, independently of every
inbound decision. See [TLS boundaries](#tls-boundaries).

### Defaults and what loosening them accepts

Every v4 default in the following table was chosen for a security reason. Changing one is
legitimate; doing so unaware of the consequence is the failure this table
exists to prevent.

| Default                                                       | Why it is the default                                                                                                                                                                                                                                                                                              |
| ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `BIND_HOST=127.0.0.1`                                         | Keeps the application listener off public interfaces in the supported edge-termination topology. Binding elsewhere exposes the application directly and is what makes the inbound-TLS acknowledgment mandatory.                                                                                                    |
...
| `AUTH_ALLOW_QUERY_SECRET=false`                               | Keeps the bearer secret out of URLs, access logs, browser history, and referrer headers. Enabling it exists only because browsers can't set headers on a WebSocket handshake.                                                                                                                                      |
| empty `ALLOWED_ORIGINS`                                       | Applies no Origin restriction, for compatibility with native clients. List exact origins for browser deployments. Startup rejects wildcards rather than honoring them.                                                                                                                                             |
| `ALLOW_MISSING_ORIGIN=false`                                  | Prevents a client from bypassing a configured Origin policy by omitting the header. Setting it true is an explicit relaxation for native clients, which are then gated only by `AUTH_MODE`.                                                                                                                        |
...
| sessions: `5` per device, `10` per IP, no global cap          | Bounds ordinary per-client abuse while preserving compatibility. The absent global cap is the notable gap: in production, set `MAX_SESSIONS_GLOBAL` to match host capacity, because per-IP limits alone don't bound a distributed client population.                                                               |
...
| `MAX_SUBNEGOTIATION_BYTES=65536`, `OUTPUT_BUFFER_BYTES=51200` | Bound per-session memory while retaining legitimate Telnet protocol payloads and enough replay history to resume. A hostile MUD can't grow either one without bound.                                                                                                                                               |
| heartbeat enabled, `30000` ms interval, `90000` ms timeout    | Reclaims sockets whose peer has vanished without a close, while tolerating two missed pings. Startup rejects a timeout at or below the interval, which would reclaim every peer.                                                                                                                                   |
...
`TARGET_MODE` determines which destinations a client can name. It is not a
...
so spelling can't evade the check.

**`allowlist`** permits only exact operator-supplied `host:port` entries. An
entry can deliberately name a private address, because an operator naming an
...
- The proxy resolves the hostname once and checks every returned address
  against reserved networks, so a client can't reach loopback, link-local, or
  private ranges through a public name.
- The proxy then dials _the validated address_, not the name. Re-resolving
  between validation and connect is the DNS-rebinding hole, and not doing so
  is what closes it.

Capacity is reserved before any DNS or TCP work, so a client can't issue many
...
protocol—go through this same authorization path. They deliberately don't
...
a caller is entitled to use the service, and nothing else. It doesn't identify
individual users, doesn't encrypt anything, and doesn't authorize a particular
MUD destination. Target policy does that, separately. Startup
enforces a minimum of 32 UTF-16 code units. Comparison is constant-time, so
timing can't reveal a wrong secret, and failed-authentication bookkeeping is
bounded so that it can't itself exhaust memory.

Header transport is preferred. `AUTH_ALLOW_QUERY_SECRET=true` additionally
accepts the secret in the query string, and exists only because browsers can't
set headers on a WebSocket handshake. URLs reach access logs, proxy
...
assertion on every upgrade. It is experimental, and it hasn't had an
independent cryptographic review. Treat it as defense in depth paired with
...
constrains which web origins a browser will let connect. A native client can
set any Origin value, so the check binds only callers that already honor the
browser security model. Matching is case-insensitive on
...
the immediate peer address is used. When set, the proxy honors forwarded headers
_only_ when the immediate peer is inside the trust list, and the walk back
through the forwarding chain stops at the first untrusted hop.

Setting this range too broadly is the failure mode. Trusting a wide range, or
`true`, lets any client inside it present its own `X-Forwarded-For` and have it
accepted.
...
Caddy's default behavior is to _append_ to a client-supplied
...
| -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| per-device and per-IP session caps           | One client population monopolizing session slots.                                                                                                                               |
...
| App Attest challenge and key stores          | Nonce issuance and attested-key registration as memory-growth vectors; both are capped and time-bounded, and saturation evicts the oldest entries rather than refusing service. |
...
**`prefer`** (default) attempts TLS and falls back to plaintext _at most once_,
on any of four triggers:

- A classified TLS negotiation error, which is evidence that the peer doesn't
  speak TLS.
- The peer closing the connection during the handshake, which is how a
  plaintext server typically answers a ClientHello.
- The four-second TLS handshake deadline expiring, which bounds a peer that
  accepts TCP and then ignores the ClientHello.
- Certificate validation failure.

Certificate validation failure deserves emphasis. Very few MUDs present a
certificate signed by a publicly trusted authority, so an untrusted or
self-signed certificate—and therefore a downgrade to plaintext—is the _common_
outcome against a TLS-capable MUD, not an edge case. `prefer` is downgradeable
by an active network attacker for the same reason: someone on the path can
induce any of these four conditions.

Errors that are not evidence about TLS—`ECONNREFUSED`, `ENOTFOUND`, and other
transport codes—fail the connection rather than triggering a fallback. The
proxy treats an unclassifiable error as not-evidence, and doesn't permit
plaintext.
...
The four-second handshake deadline is distinct from the 12-second Telnet
...
the address dialed, so validation still means something in `arbitrary` mode
where the two differ. The proxy dials a target given as a bare IP literal
without SNI, because an IP is not a valid server name.
...
- Independent cryptographic assurance for the experimental App Attest verifier.
  No one has reviewed its implementation, and this document makes no claim that
  attacks against it are absent.
...
- _mud-web-proxy_ owns runtime target authorization, configured
  ...
  rate limits, and outbound TLS-mode behavior.
...
The other owners don't provide a control listed under one owner. In particular,
...
- **Restarts discard state.** Sessions, resume buffers, and rate-limiter
  windows don't survive a restart.
...
- **There is no upstream certificate-trust configuration.** No custom CA, no
  ...
  mechanism that an operator can apply to the process; it is not a feature of this
...
- **The proxy relays untrusted MUD output verbatim.** The client must render it
  safely.
- **No global session cap by default.** Per-IP limits don't bound a
  distributed client population; set `MAX_SESSIONS_GLOBAL` in production.

## Evidence and regression-coverage ledger

Every material claim in this document maps to an implementation path and, where one
...
| ------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| Inbound TLS fails closed; non-loopback plaintext needs acknowledgment; broken certificate material aborts startup                    | `src/runtime-config.ts`, `wsproxy.ts`                                         | `tests/config-security-guards.test.ts`, `tests/inbound-tls-material.test.ts`                                                                                                                    | None                                                                                                  |
| Fixed and allowlist targets deny anything not explicitly permitted, including case and trailing-dot variants                         | `src/runtime-config.ts`, `src/target-policy.ts`, `src/session-integration.ts` | `tests/target-policy-modes.test.ts`, `tests/config-security-guards.test.ts`                                                                                                                     | None                                                                                                  |
| Arbitrary targets require enforced auth and allowed ports, reject reserved resolution results, and dial the validated address        | `src/runtime-config.ts`, `src/target-policy.ts`, `src/session-integration.ts` | `tests/target-mode-guard.test.ts`, `tests/target-policy-modes.test.ts`, `tests/connect-path-dns.test.ts`                                                                                        | None                                                                                                  |
| Shared-secret upgrade authorization, query opt-in, and bounded failed-auth tracking                                                  | `src/wsproxy-utils.ts`, `wsproxy.ts`                                          | `tests/shared-secret-auth.test.ts`, `tests/config-security-guards.test.ts`                                                                                                                      | None                                                                                                  |
...
| Exact Origin policy, wildcard rejection, and missing-Origin behavior                                                                 | `src/runtime-config.ts`, `src/wsproxy-utils.ts`, `wsproxy.ts`                 | `tests/origin-checking.test.ts`                                                                                                                                                                 | None                                                                                                  |
| Forwarded identity is accepted only from configured peers, and per-IP accounting follows it                                          | `src/runtime-config.ts`, `src/wsproxy-utils.ts`, `wsproxy.ts`                 | `tests/trusted-proxy.test.ts`, `tests/trusted-proxy-config.test.ts`, `tests/trusted-proxy-startup.test.ts`, `tests/ip-counting.test.ts`                                                         | None                                                                                                  |
| The systemd topology keeps plaintext on loopback behind one trusted hop and overwrites both client-IP headers                        | `config/mud-web-proxy.env.systemd.example`, `deploy/caddy/Caddyfile.example`  | `tests/deployment/systemd-contract.test.ts`                                                                                                                                                     | None                                                                                                  |
...
| Pending, per-IP, per-device, global, and clientless-session capacity is bounded and released                                         | `src/session-manager.ts`, `src/session-integration.ts`                        | `tests/pending-dial-reservation.test.ts`, `tests/dial-reservation-handoff.test.ts`, `tests/ip-counting.test.ts`, `tests/global-session-cap.test.ts`, `tests/clientless-session-reaping.test.ts` | None                                                                                                  |
| Per-connection and per-address frame rates are both enforced with bounded bookkeeping                                                | `src/message-rate-limit.ts`, `wsproxy.ts`                                     | `tests/message-rate-limit.test.ts`                                                                                                                                                              | None                                                                                                  |
| Telnet subnegotiation and resume history have byte caps                                                                              | `src/telnet-parser.ts`, `src/circular-buffer.ts`, `src/session.ts`            | `tests/telnet-subneg-cap.test.ts`, `tests/circular-buffer-cap.test.ts`                                                                                                                          | None                                                                                                  |
| Heartbeat reclaims silent peers exactly once                                                                                         | `src/heartbeat.ts`, `wsproxy.ts`                                              | `tests/heartbeat.test.ts`                                                                                                                                                                       | None                                                                                                  |
| Required MUD TLS never downgrades; preferred TLS falls back at most once on a classified trigger; both protocols share one connector | `src/mud-transport.ts`, `src/session.ts`, `wsproxy.ts`                        | `tests/mud-transport.test.ts`, `tests/mud-tls-mode.test.ts`, `tests/tls-servername.test.ts`, `tests/session-lifecycle.test.ts`, `tests/e2e/legacy-protocol.test.ts`                             | None                                                                                                  |
| Diagnostics are disabled by default and accept the admin token only when enabled                                                     | `src/runtime-config.ts`, `src/wsproxy-utils.ts`, `wsproxy.ts`                 | `tests/open-source-regressions.test.ts`, `tests/wsproxy-utils.test.ts`                                                                                                                          | None                                                                                                  |
| Logs redact configured secrets and neutralize hostile control text                                                                   | `src/log-redaction.ts`, `wsproxy.ts`                                          | `tests/log-redaction.test.ts`                                                                                                                                                                   | None                                                                                                  |
...
| Ordered shutdown has an absolute deadline and flushes state                                                                          | `src/shutdown.ts`, `wsproxy.ts`                                               | `tests/shutdown.test.ts`                                                                                                                                                                        | None                                                                                                  |
